...
export async function applySigningRequestAction(input: {
  ...
  if (input.action === "resend_whatsapp") {
    if (!req.phone) throw new Error("No phone on file");
    if (!req.signingUrl) throw new Error("No signing link");
    if (!req.deliveryChannels.includes("whatsapp")) throw new Error("WhatsApp was not enabled for this request");
    if (req.whatsappConsentConfirmed !== true) throw new Error("WhatsApp consent not recorded");
    ...
    if (!wa) throw new Error("WhatsApp not sent");
    await appendRelayEvent({
      signingRequestId: req.id,
      type: "whatsapp_sent",
      message: "WhatsApp resent (mock Twilio)",
      metadata: { sid: wa.sid, to: wa.to, body: wa.body, manual: true },
      createdBy: input.actor.sub,
    });
    await store.upsertSigningRequest({ ...req, updatedAt: nowIso() });
    return { ok: true as const };
