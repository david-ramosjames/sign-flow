    ...
    <div className="space-y-6">
      ...
      <div className="rounded-2xl border border-[color:var(--border)] bg-[color:var(--card)] p-6 shadow-sm">
        ...
        <div className="mt-4 text-xs text-slate-600">
          ...
          optional <code>SIGNFLOW_ALLOWED_GOOGLE_EMAILS</code> (empty = any{" "}
          <code className="text-xs">@ramosjames.com</code>; or set domains/emails;{" "}
          <code className="text-xs">*</code> = any Google account), admin deletes via{" "}
          <code>SIGNFLOW_ADMIN_EMAILS</code>.
      ...
      <div className="rounded-2xl border border-[color:var(--border)] bg-[color:var(--card)] p-6 shadow-sm">
        ...
        <p className="mt-2 text-sm text-slate-700">
          ...
          minutes and sends any due reminders. Timing and SMS copy come from Admin → Messages & reminders — there is a
          contract sequence and a general sequence (HIPAA, SAR, Disbursement). Same-day nudge defaults to 30 minutes
          after send. Client reminders only go out between <strong>7:00 AM and 8:00 PM US Central</strong>; overnight due
          items wait until 7:00 AM. Opening the <strong>Signing requests</strong> dashboard also triggers a backup sweep.
          To run the endpoint manually, GET that URL with header <code className="text-xs">x-cron-secret</code> if{" "}
