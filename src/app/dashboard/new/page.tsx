  ...
  return (
    <div className="space-y-6">
      <div className="flex items-start justify-between gap-4">
        <div>
          <h1 className="text-2xl font-semibold tracking-tight text-slate-900">New retainer request</h1>
              ...
              <label className="flex items-center gap-2 text-sm text-slate-700">
                <input
                  ...
                  onChange={(e) => {
                    ...
                    setChannels((c) => ({ ...c, whatsapp: on }));
                    if (!on) setWhatsappConsent(false);
                  }}
                />
                WhatsApp (Twilio; same number as SMS when both are used)
              </label>
            </div>
          </div>
          ...
          {channels.whatsapp ? (
            <label className="flex items-start gap-3 rounded-xl border border-slate-200 bg-slate-50 p-4 text-sm text-slate-800">
              <input
                ...
                required={channels.whatsapp}
              />
              <span>
                Lead has opted in to receive WhatsApp messages from Ramos James Law about their retainer/signing link on
                this number.
                <span className="mt-1 block text-xs text-slate-600">Required before sending WhatsApp (separate from SMS consent).</span>
              </span>
            </label>
          ) : null}
        ...
        <div className="space-y-4 rounded-2xl border border-[color:var(--border)] bg-[color:var(--card)] p-5 shadow-sm lg:col-span-2">
          ...
          <div className="rounded-xl border border-slate-200 bg-white p-4 text-sm text-slate-800">
            <div className="text-xs font-semibold uppercase tracking-wide text-slate-500">
              {channels.sms && channels.whatsapp ? "SMS & WhatsApp" : channels.whatsapp ? "WhatsApp" : "SMS"}
            </div>
            {channels.sms || channels.whatsapp ? (
              <p className="mt-2 whitespace-pre-wrap leading-relaxed">{smsPreview}</p>
            ) : (
              <p className="mt-2 text-slate-500">Select SMS and/or WhatsApp to preview the outbound message.</p>
            )}
          </div>
