  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        <div>
          <h1 className="text-2xl font-semibold tracking-tight text-slate-900">Retainer signing</h1>
          <p className="mt-1 text-sm text-[color:var(--muted)]">
            Send retainer agreements through Adobe. Follow up by SMS or WhatsApp when email is not enough.
          </p>
          {data?.store === "mock" ? (
              ...
              {rows.length === 0 ? (
                <tr>
                  <td className="px-4 py-10 text-sm text-slate-600" colSpan={10}>
                    No requests yet. Click <span className="font-medium">Send retainer</span> to create one.
                  </td>
                </tr>
              ) : (
                rows.map((r) => {
